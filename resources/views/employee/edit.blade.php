...
<div class="row">
    <div class="col-12 col-md-12 col-lg-12">
        ...
        <div class="card">
            ...
            <form id="employeeForm" name="employeeForm" method="POST" action="{{ route('employee.edit', $employee->uid) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="row">
                        <div class="form-group col-2">
                            <label>UID</label>
                            <input type="text" class="form-control" id="uid" name="uid" value="{{ old('uid', $employee->uid) }}">
                        </div>
                        ...
                        <div class="form-group col-4">
                            <label>Email</label>
                            <input type="email" class="form-control" id="email" name="email" value="{{ old('name', $employee->email) }}">
                        </div>
                        <div class="form-group col-2">
                            <label>Date of Birth</label>
                            <input type="text" class="form-control datepickerEmp" id="dateOfBirth" name="dateOfBirth" value="{{ old('dateOfBirth', $employee->dateOfBirth) }}">
                        </div>
                        <div class="form-group col-2">
                            <label>Gender</label>
                            <select id="gender" name="gender" class="form-control select2 select2-hidden-accessible" tabindex="-1" aria-hidden="true">
                                ...
                                <option {{('F' === $employee->gender) ? 'Selected' : ''}} value="F">Female</option>
                            </select>
                        </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-3">
                            <label>Photo</label>
                            <input type="file" class="form-control" id="photo" name="photo" value="{{ old('photo', $employee->photo) }}">
                        </div>
                        <div class="form-group col-3">
                            ...
                            <select id="department_id" name="department_id" class="form-control select2 select2-hidden-accessible" tabindex="-1" aria-hidden="true">
                                ...
                                <option {{($dp->department_id === $employee->department_id) ? 'Selected' : ''}} value="{{ $dp->department_id }}">{{ $dp->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-3">
                            ...
                            <select id="position_id" name="position_id" class="form-control select2 select2-hidden-accessible" tabindex="-1" aria-hidden="true">
                                ...
                                <option {{($ps->position_id === $employee->position_id) ? 'Selected' : ''}} value="{{ $ps->position_id }}">{{ $ps->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-3">
